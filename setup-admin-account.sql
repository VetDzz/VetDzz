-- SIMPLE ADMIN SETUP FOR SIHAAEXPRESS
-- Just create the admin account - no complex user ID management needed

-- STEP 1: Go to Supabase Dashboard > Authentication > Users
-- Click "Invite User" and add:
-- Email: glowyboy01@gmail.com
-- Password: Mindup2019
-- IMPORTANT: Email verification will be required for login
-- That's it! The admin panel will recognize this Gmail automatically.
...
-- Enable RLS on banned_users (only if table was just created)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE tablename = 'banned_users'
    AND policyname = 'Admin can manage bans'
  ) THEN
...
-- Create function to create banned_users table (for admin panel)
CREATE OR REPLACE FUNCTION create_banned_users_table()
RETURNS void AS $$
BEGIN
  -- This function does nothing if table already exists
  CREATE TABLE IF NOT EXISTS banned_users (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    user_id UUID NOT NULL,
    banned_until TIMESTAMP WITH TIME ZONE NOT NULL,
...
-- OPTIONAL: Create a simple admin tracking table
...
-- Add your admin Gmail to the tracking table
INSERT INTO admin_emails (email)
VALUES ('glowyboy01@gmail.com')
...
-- That's all! Your admin account is ready with complete deletion capabilities.
-- The admin panel checks for your specific email address for access.
